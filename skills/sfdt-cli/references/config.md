...
- **`config.json` is committed, so it is untrusted input.** `loadConfig()` refuses settings by capability and says what it refused: code-execution keys (`plugins[]`, `pluginOptions.autoDiscover`), process-spawn keys (`mcp.salesforce.command`/`args`), privilege keys (`ai.agent.enabled`/`allowWrite`, both now inert), destinations (a non-loopback `ai.baseURL`, a channel's literal `webhookUrl`/`url`), and any path key — `logDir`, `manifestDir`, `docs.outputDir`, `monitoring.backupDir`, `data.dir`, `scratch.definitionFile`, `deployment.smart.noOverwriteManifest`, … — whose value is absolute or escapes the project root. `SFDT_ALLOW_UNSAFE_CONFIG=1` opts a shell back in. Prefer the `*Env` forms: they are never refused, because the value comes from your shell rather than the repo.
- **The write-capable AI auto-fix loop is granted by `SFDT_ALLOW_AI_WRITE=1`, not by config.** `ai.agent.enabled`/`allowWrite` are accepted by the schema for backward compatibility and ignored. `.sfdt/prompts.json` overrides are likewise ignored unless `SFDT_ALLOW_UNSAFE_CONFIG=1` — a prompt is the instruction text sent to your AI provider, and that file ships with the clone.
